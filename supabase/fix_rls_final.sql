-- FIX: Infinite recursion in get_user_role() + broken RLS policies
-- Run this in Supabase SQL Editor (it handles everything safely in order)

-- STEP 1: Drop ALL policies (removes dependencies on get_user_role)
DO $$ DECLARE r RECORD; BEGIN
  FOR r IN (
    SELECT policyname, tablename, schemaname
    FROM pg_policies
    WHERE schemaname = 'public' OR (schemaname = 'pg_catalog' AND tablename = 'objects')
  ) LOOP
    EXECUTE format('DROP POLICY IF EXISTS %I ON %I.%I', r.policyname, r.schemaname, r.tablename);
  END LOOP;
END $$;

-- Also drop policies in storage schema
DO $$ DECLARE r RECORD; BEGIN
  FOR r IN (
...
-- STEP 2: Drop and recreate get_user_role with search_path = '' to prevent recursion
